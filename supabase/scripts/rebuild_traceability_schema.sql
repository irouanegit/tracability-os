...
-- Time-aware production planning with explicit raw-lot and semi-finished dependencies.
...
create table if not exists production_plan_series (
  ...
  planned_time time not null default time '06:30',
...
alter table production_plan_series add column if not exists planned_time time not null default time '06:30';
...
alter table production_plans add column if not exists planned_time time not null default time '06:30';
...
-- 034_delivery_confirmations.sql
...
-- 025_add_chocolat_aiguebelle_to_chocolate_substitution.sql
alter table products
  add column if not exists substitution_group text;
...
create table if not exists public.deliveries (
  ...
  constraint deliveries_store_name_check
    check (store_name in ('AL QODS', 'MIMOUZA', 'CHEFCHAOUNI', 'MOHAMMEDIA', 'ORCHIDÉE')),
  constraint deliveries_delivery_number_check check (delivery_number between 1 and 4),
  constraint deliveries_status_check check (status in ('confirmed', 'cancelled')),
  constraint deliveries_confirmed_product_count_check check (confirmed_product_count >= 0),
  constraint deliveries_slot_key unique (delivery_date, store_name, delivery_number)
);
...
-- Keep clean rebuilds aligned with migration 037.
