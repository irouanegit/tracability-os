-- Migration 040: Add purchase price HT (MAD) option to receptions and lots

-- 1. Add nullable unit_price_ht column to reception tables and lots
alter table if exists reception_batches
  add column if not exists unit_price_ht numeric;

alter table if exists raw_material_receptions
  add column if not exists unit_price_ht numeric;

alter table if exists lots
  add column if not exists unit_price_ht numeric;

-- 2. Update reception_batch_lines view to include unit_price_ht
...
-- 3. Update reception_batch_history view to include total_price_ht and unit_price_ht
...
select
  ...
  sum(case when r.unit_price_ht is not null then r.quantity * r.unit_price_ht else null end) as total_price_ht,
  b.unit_price_ht
...
-- 4. Update recent_raw_material_receptions view
...
-- 5. Update create_raw_material_reception_batch preserving audit trail and adding unit_price_ht
...
begin
  ...
  loop
    ...
    v_unit_price_ht := nullif(v_line->>'unit_price_ht', '')::numeric;
    ...
    if v_unit_price_ht is not null and v_unit_price_ht < 0 then
...
-- 6. Update update_raw_material_reception_batch preserving audit trail and adding unit_price_ht
...
-- 7. Update create_raw_material_reception supporting unit_price_ht
create or replace function create_raw_material_reception(
  ...
  p_observations text,
  p_unit_price_ht numeric default null
)
...
declare
  ...
  v_reception raw_material_receptions;
begin
  if p_unit_price_ht is not null and p_unit_price_ht < 0 then
    raise exception 'Unit price cannot be negative.';
  end if;
...
-- 8. Grant permissions aligned with security model
grant select on reception_batch_lines to authenticated;
grant select on reception_batch_history to authenticated;
grant select on recent_raw_material_receptions to authenticated;
...
grant execute on function create_raw_material_reception(timestamptz, uuid, uuid, text, numeric, text, date, numeric, quality_status, quality_status, text, text, text, numeric) to authenticated;
